...
const guessNextShortcutCell = (): [number, number] => {
    const occupied = new Set(
        ([]).map((item) => `${Math.round(item?.cell?.[0] || 0)}:${Math.round(item?.cell?.[1] || 0)}`)
    );
    const maxRows = 12;
    const maxCols = 8;
...
export const CwViewExplorer = createViewConstructor(TAG, (Base: any)=>{
    ...
    render(options?: ExplorerOptions): HTMLElement {
        if (options) {
            ...
            this.initialPath = this.options.path;
        }
