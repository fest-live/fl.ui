...
export class BoxWithSidebar extends UIElement {
    ...
    render = function () {
        return H`<div class="bar c2-surface"><button part="open-sidebar" class="open-sidebar c2-surface" on:click=${() => { this.sidebarOpened.value = !this.sidebarOpened.value; }}><ui-icon icon="${conditional(this.sidebarOpened, 'text-outdent', 'list')}"></ui-icon></button><slot name="bar"></slot></div>
