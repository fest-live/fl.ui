...
/** Split Inter / InterDisplay / InterVariable so no single CRX chunk exceeds 2048 kB. */
export function fontRegistryShardId(key: string, family: string, style?: string | number): string {
    const italic = String(style || "").toLowerCase() === "italic" || /italic/i.test(key);
    if (family === "InterVariable") return "variable";
    if (family === "InterDisplay") return italic ? "display-italic" : "display-roman";
    return italic ? "inter-italic" : "inter-roman";
}

const FONT_REGISTRY_SHARDS = [
    "variable",
    "inter-roman",
    "inter-italic",
    "display-roman",
...
function formatFontRegistryObject(entries: FontRegistryEntry[]): string {
    return `export const fontRegistry: Record<string, any> = {
${entries.map(({ key, metadata }) => `    '${key}': {
...
export async function writeFontRegistryShards(
    outputFile: string,
    generatedFonts: FontRegistryEntry[],
): Promise<void> {
    const grouped = new Map<string, FontRegistryEntry[]>();
    for (const id of FONT_REGISTRY_SHARDS) grouped.set(id, []);
    for (const font of generatedFonts) {
        const id = fontRegistryShardId(font.key, font.metadata.family, font.metadata.style);
    ...
    for (const id of FONT_REGISTRY_SHARDS) {
        const entries = grouped.get(id) || [];
        const shardPath = resolve(dir, `font-registry-${id}.ts`);
        await writeFile(
            shardPath,
            `/**
 * Font Registry shard (${id})
 *
 * Auto-generated by vite-plugin-font-encoder
 * DO NOT EDIT MANUALLY
 */

${formatFontRegistryObject(entries)}`,
            "utf-8",
        );
    }
...
export function fontEncoder(options: FontEncoderOptions): Plugin {
    ...
    return {
        ...
        async buildStart() {
            ...
            await writeFontRegistryShards(outputFile, generatedFonts);
